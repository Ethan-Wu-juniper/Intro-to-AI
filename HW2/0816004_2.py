...
class Node:
  def __init__(self, gini, sample_num, sample_num_per_class, predicted_class):
      self.gini = gini
...
class Model:
  ...
  def _grow_tree(self, X, y, depth=0):
    sample_num_per_class = [0,0]
    for c in range(self.n_classes_):
      for i in range(y.size):
        if y[i]==c:
          sample_num_per_class[c] += 1
    predicted_class = np.argmax(sample_num_per_class)
    node = Node(
          gini=1.0 - sum((n / y.size) ** 2 for n in sample_num_per_class),
          sample_num=y.size,
  ...
  def predict(self, X: np.ndarray) -> np.ndarray:
    return [self.to_predict(inputs) for inputs in X]
  def to_predict(self, inputs):
    node = self.tree_
    while node.left or node.right:
      if node.left and inputs[node.feature_index] < node.threshold:
        node = node.left
      elif node.right:
        node = node.right
